...
use super::{LocalTranslationModel, TranslationLanguage};
...
pub struct LocalTranslationService {
...
impl LocalTranslationService {
    ...
    /// Translates text with a lazily loaded model and bounded result cache.
    ///
    /// # Errors
    ///
    /// Returns an error when model loading, tokenization, or inference fails.
    ///
    /// # Panics
    ///
    /// Panics if the internal service mutex is poisoned or the engine registry invariant is
    /// violated.
    pub fn translate(
        &self,
        model_dir: PathBuf,
...
impl Default for LocalTranslationService {
    fn default() -> Self {
        Self::new()
    }
}
