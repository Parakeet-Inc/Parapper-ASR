// Token/tensor dimensions are model-bounded; benchmark ratios intentionally use f64.
#![allow(
...
fn decoder_input_tokens<'a>(
    ...
    generated_token_ids
        .last()
        .map_or(prompt_token_ids, std::slice::from_ref)
}
...
fn decoder_total_sequence_len(
    ...
    decoder_state.map_or(input_token_count, |state| {
        state.total_sequence_len + input_token_count
    })
}
...
mod tests {
    ...
    #[allow(
        ...
        reason = "the benchmark corpus is clearer as one literal case table"
    )]
