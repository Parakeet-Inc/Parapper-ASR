...
pub struct SupertonicOnnxTtsEngine {
...
impl SupertonicOnnxTtsEngine {
    /// Loads all Supertonic component sessions and the selected speaker style.
    ///
    /// # Errors
    ///
    /// Returns an error when configuration, style, Unicode metadata, or an ONNX session cannot be
    /// loaded.
    pub fn new(
        model_dir: &Path,
        speaker_id: Option<i32>,
    ...
    /// Synthesizes one utterance, updating speaker style when requested.
    ///
    /// # Errors
    ///
    /// Returns an error when speaker assets, text preprocessing, or model inference fails.
    pub fn synthesize(
        &mut self,
        text: &str,
        speaker_id: Option<i32>,
...
mod tests {
    ...
    use crate::{init_onnx_runtime, tts::SUPERTONIC3_LANGUAGE_CODES};
