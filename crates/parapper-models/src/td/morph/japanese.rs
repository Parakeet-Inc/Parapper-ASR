...
use crate::{
    asr::AsrTranscript,
    td::{
        GrammarBoundaryClass, TurnBoundaryCandidate, audio_window::audio_window_for_boundary,
        boundary::sample_end_for_char_end,
    },
    vad::VadResult,
};
...
impl JapaneseMorphAnalyzer {
    /// Loads a Japanese morphology dictionary from an in-memory or streaming reader.
    ///
    /// # Errors
    ///
    /// Returns an error when the dictionary cannot be read or decoded.
    pub fn from_dictionary_reader(reader: impl Read) -> Result<Self> {
        let dict =
            Dictionary::read(reader).context("Failed to read embedded Vibrato dictionary")?;
        Ok(Self {
            tokenizer: Tokenizer::new(dict),
        })
    }

    /// Loads a memory-mapped Japanese morphology dictionary.
    ///
    /// # Errors
    ///
    /// Returns an error when the dictionary cannot be read or decoded.
    pub fn from_dictionary_path(path: &Path) -> Result<Self> {
