...
impl NamoTurnDetectorEngine {
    /// Loads a Namo turn-detector model and tokenizer.
    ///
    /// # Errors
    ///
    /// Returns an error when model/tokenizer artifacts are missing or the ONNX session fails.
    ...
    /// Classifies whether the supplied text ends the current turn.
    ///
    /// # Errors
    ///
    /// Returns an error when tokenization or ONNX inference fails, or model output is invalid.
