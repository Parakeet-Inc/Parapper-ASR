...
pub trait NoiseCancellationEngine: Send {
    ...
    /// # Errors
    ///
    /// Returns an error when the concrete model cannot process the chunk.
    fn process(&mut self, samples: &[f32]) -> Result<Vec<f32>>;

    /// Returns the streaming output delay relative to the input timeline.
    ///
    /// The caller uses this value when VAD consumes enhanced audio while ASR
    /// consumes the corresponding unmodified audio.
...
impl UlUnasNoiseCancellationEngine {
    /// Loads the UL-UNAS noise-cancellation model.
    ///
    /// # Errors
    ///
    /// Returns an error when the model artifact is missing or its ONNX session cannot be loaded.
...
impl NoiseCancellationEngine for UlUnasNoiseCancellationEngine {
    ...
    fn output_delay_samples(&self) -> usize {
        // One 256-sample STFT hop, or 16 ms at the fixed 16 kHz NC rate.
        HOP_SIZE
    }
}
...
fn pop_hop(overlap: &mut VecDeque<f32>, norm: &mut VecDeque<f32>) -> Vec<f32> {
    ...
    for _ in 0..HOP_SIZE {
        let sample = overlap
            .pop_front()
            .expect("overlap buffer must contain one FFT frame");
        let weight = norm
            .pop_front()
            .expect("normalization buffer must contain one FFT frame");
