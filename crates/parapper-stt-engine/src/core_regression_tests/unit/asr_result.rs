// Test fixtures use small integer frame numbers expressed as f32 timestamps.
...
#[test]
fn new_turn_target_allocates_id_only_on_first_usable_result_after_failed_result() {
    let mut builder = RecognitionSessionTestBuilder::new().turn_detector(TurnDetector::Simple);
    let asr_handle = builder.use_manual_asr();
    let outputs = builder.use_recording_phrase_sink();
    let (mut runtime, _config) = builder.build();
    let mut failed_request = interim_request_for_turn(1, 1);
    failed_request.target.turn_target = crate::transcription::task::AsrTurnTarget::New;
    runtime_state(&mut runtime).in_flight(failed_request.clone());
    asr_handle.fail_request(&failed_request);
    runtime.step();

    assert!(runtime.turn_store.turns.is_empty());
    assert_eq!(runtime.counters.next_turn_id, 1);

    let mut usable_request = failed_request;
    usable_request.request_id = AsrRequestId(2);
    runtime_state(&mut runtime).in_flight(usable_request.clone());
    asr_handle.complete_request_with_text(&usable_request, "最初の有効結果");
    runtime.step();

    assert_eq!(runtime.counters.next_turn_id, 2);
    assert_eq!(runtime.turn_store.turns.len(), 1);
    assert_eq!(
        outputs.lock().expect("outputs should be readable")[0].turn_id,
        1
    );

    let mut repeated_request = usable_request;
    repeated_request.request_id = AsrRequestId(3);
    repeated_request.kind = AsrTaskKind::CompletionCheck;
    repeated_request.target.last_segment_id = Some(SegmentId(1));
    repeated_request.close_reason = Some(SegmentCloseReason::EndSilenceReached);
    runtime_state(&mut runtime).in_flight(repeated_request.clone());
    asr_handle.fail_request(&repeated_request);
    runtime.step();
    assert_eq!(runtime.counters.next_turn_id, 2);
    ...
    assert!(
        outputs
            ...
            .unwrap()
            .is_final,
        "failed completion on a resolved New correlation must finalize the existing draft"
    );
}
