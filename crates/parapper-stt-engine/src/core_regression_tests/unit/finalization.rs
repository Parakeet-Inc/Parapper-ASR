...
fn turn_runtime_output_identity_includes_turn_session_id() {
    ...
    assert_eq!(
        outputs[0].source_session,
        SourceSessionKey::legacy_single_source(77)
    );
    assert_eq!(
        outputs[0].source_identity,
        SourceIdentitySnapshot::legacy_single_source()
    );
}

#[test]
fn source_identity_snapshot_is_unchanged_from_interim_to_final_output() {
    let identity = SourceIdentitySnapshot::new(
        "desk-mic-channel-2".into(),
        "Speaker B".into(),
        "usb-interface-1".into(),
        Some(2),
    );
    let mut builder = RecognitionSessionTestBuilder::new()
        .turn_session_id(77)
    ...
    runtime.push_vad_frame(&[1.0; 512], vad(true));
    runtime.step();
    runtime.push_vad_frame(&[0.0; 512], vad(false));
    runtime.step();
    runtime.push_vad_frame(&[1.0; 512], vad(true));
    runtime.step();
    let request = runtime
        ...
        .in_flight_request
        .clone()
        .expect("SegmentStarted -> SegmentExtended -> SegmentClosed must dispatch interim ASR");
    assert_eq!(request.kind, AsrTaskKind::InterimDisplay);
    asr_handle.complete_request_with_text(&request, "channel two first");
    runtime.step();
    ...
    let request = runtime
        ...
        .in_flight_request
        .clone()
        .expect("end-of-turn silence must dispatch completion ASR");
    asr_handle.complete_request_with_text(&request, "channel two final");
    runtime.step();
    ...
    assert_eq!(outputs[0].source_identity, identity);
    assert_eq!(outputs[1].source_identity, identity);
    assert_eq!(
        outputs[0].source_session,
        SourceSessionKey::new(77, "desk-mic-channel-2".into())
    );
    assert_eq!(outputs[1].source_session, outputs[0].source_session);
}
