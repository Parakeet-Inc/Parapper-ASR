...
use crate::{RecognitionConfig as TestConfig, VadResult};

macro_rules! recognition_config {
...
fn update_config_shortens_turn_check_threshold_during_active() {
    let initial_config = recognition_config! {
        vad_interval_ms: CHUNK_MS,
        turn_check_silence_ms: FOUR_CHUNKS_MS,
        segment_start_speech_ms: START_IMMEDIATELY_MS,
        ..TestConfig::default()
    };
    let next_config = recognition_config! {
        vad_interval_ms: CHUNK_MS,
