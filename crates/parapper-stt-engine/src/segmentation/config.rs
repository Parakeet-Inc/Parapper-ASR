...
impl SegmentBuilderConfig {
    pub(super) fn from_config(config: &RecognitionConfig) -> Self {
        Self {
            segment_start_threshold: chunks_for_millis(
                config.segment_start_speech_ms,
                config.vad_interval_ms,
            ),
            interim_result_threshold: (config.interim_result_enabled
                && !config.streaming_interim_asr_enabled)
                .then(|| {
                    chunks_for_millis(config.interim_result_silence_ms, config.vad_interval_ms)
                }),
            turn_check_threshold: chunks_for_millis(
                config.turn_check_silence_ms,
                config.vad_interval_ms,
            ),
...
fn pre_speech_max_chunks(config: &RecognitionConfig) -> usize {
    chunks_for_millis(config.turn_check_silence_ms, config.vad_interval_ms) as usize
}
