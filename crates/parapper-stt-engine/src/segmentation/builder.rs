...
#[cfg(test)]
#[path = "builder_regression_tests.rs"]
...
impl SegmentBuilder {
    ...
    pub fn push(&mut self, samples: &[f32], vad_result: VadResult) -> Vec<SegmentBuilderEvent> {
    ...
    pub fn flush(&mut self) -> Vec<SegmentBuilderEvent> {
        let state = std::mem::replace(&mut self.state, SegmentBuilderState::new(&self.config));
        match state {
            SegmentBuilderState::Active(active) => {
                let next_state =
