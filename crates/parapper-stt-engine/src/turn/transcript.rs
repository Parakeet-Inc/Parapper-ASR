use crate::{
    ...
    transcription::task::AsrTurnTarget,
    transcription::task::{AsrRequest, AsrTaskKind, AudioRange, GlobalSampleIndex},
    turn::Turn,
    turn_event_id,
};
use parapper_models::{asr::AsrTranscript, vad::VadResult};

impl RecognitionSession {
    pub fn apply_segment_transcript(
        &mut self,
        request: &AsrRequest,
        transcript: AsrTranscript,
        elapsed_millis: u128,
    ) -> u64 {
        let turn_id = match request.target.turn_target {
            AsrTurnTarget::New => {
                let correlation = (
                    request.target.source_session.clone(),
                    request.target.first_segment_id,
                );
                if let Some(turn_id) = self.turn_store.pending_new_turns.get(&correlation) {
                    *turn_id
                } else {
                    // A request may have been built while a prior turn is still
                    // present/finalizing. Skip those source-local ids without
                    // treating the skipped values as request-time reservations.
                    let mut turn_id = self.counters.next_turn_id;
                    while self.turn_store.turns.contains_key(&turn_id)
                        || self.turn_store.finalized_turns.contains(&turn_id)
                    {
                        turn_id = turn_id.saturating_add(1);
                    }
                    self.counters.next_turn_id = turn_id.saturating_add(1);
                    self.turn_store
                        .pending_new_turns
                        .insert(correlation, turn_id);
                    turn_id
                }
            }
            AsrTurnTarget::Existing(turn_id) => {
                self.counters.next_turn_id =
                    self.counters.next_turn_id.max(turn_id.0.saturating_add(1));
                turn_id.0
            }
        };
        ...
        self.apply_transcript_to_turn_draft(
            turn_id,
            request,
            transcript,
            elapsed_millis,
            streaming_interim_overlap_offset,
    ...
    fn apply_transcript_to_turn_draft(
        &mut self,
        turn_id: u64,
        request: &AsrRequest,
        transcript: AsrTranscript,
        ...
        replaces_streaming_interim: bool,
    ) {
        ...
        let segment_id = request.target.last_segment_id.map_or_else(
            || request.target.first_segment_id.map_or(turn_id, |id| id.0),
            |id| id.0,
        );
        let previous_segment_id = request
            .target
            .first_segment_id
            .and_then(|id| (Some(id) != request.target.last_segment_id).then_some(id.0));
        ...
        let replace_latest = draft.latest_segment_id == Some(segment_id)
        ...
        let append_start = overlap_offset.unwrap_or(0);
        let append_vad = if append_start == 0 {
            request.source_vad_results.clone()
        } else {
            vad_suffix_after_sample(
                request.source_audio.len(),
                &request.source_vad_results,
                append_start,
            )
        ...
        draft.append_recognized_segment(
            segment_id,
            previous_segment_id,
            &request.source_audio[append_start..],
            &append_vad,
            request.route,
            transcript.text,
            elapsed_millis,
    ...
    pub fn apply_rerecognition_transcript(
        &mut self,
        request: &AsrRequest,
        transcript: AsrTranscript,
        elapsed_millis: u128,
        ...
        let candidates = refresh_boundary_candidates.then(|| {
            self.io
                .boundary_detector
                .as_ref()
                .map_or_else(Vec::new, |detector| {
                    detector.candidates_for_transcript(
