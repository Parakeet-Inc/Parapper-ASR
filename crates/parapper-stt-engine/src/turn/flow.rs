use crate::{
    RecognitionSession, SegmentCloseReason, SegmentId, TurnId,
    runtime::PendingFinalization,
    take_next_output_sequence,
    transcription::{
        input::{AsrRequestEdgePadding, ensure_asr_request_edge_silence},
        route::{RecognitionRoute, configured_split_route},
        task::{
            AsrInFlight, AsrRequest, AsrRequestId, AsrTarget, AsrTaskKind, AudioRange,
            GlobalSampleIndex, TurnRevision, VadFrameIndex,
        },
    },
    turn::{RerecognitionPurpose, completion, namo, silence, timeout},
};

impl RecognitionSession {
    pub fn complete_turn_without_grammar(&mut self, turn_id: u64) {
    ...
    #[must_use]
    pub fn rerecognition_purpose_after_completion(&self) -> Option<RerecognitionPurpose> {
        completion::rerecognition_purpose(
            self.config.turn.detector,
            self.config.turn.rerecognize_full_on_complete,
        )
    }

    pub fn dispatch_rerecognition_for_turn_if_idle(
        &mut self,
        turn_id: u64,
    ...
    pub fn complete_or_continue_turn_with_namo(&mut self, turn_id: u64) {
    ...
    pub fn namo_final_decision_for_text(&mut self, turn_id: u64, text: &str) -> bool {
        let text = text.trim();
        if text.is_empty() {
            return false;
        }
    ...
    pub fn finalize_timeout_turn_after_rerecognition(&mut self, turn_id: u64) {
        if self.defer_finalization_if_blocked(PendingFinalization::new(turn_id)) {
            return;
        }
        self.finalize_turn_now(turn_id);
    ...
    pub fn clear_open_turn(&mut self) {
    ...
    pub fn handle_turn_check_silence_reached(&mut self, previous_segment_id: u64) -> bool {
    ...
    pub fn emit_stale_turn_finals(&mut self, before_turn_id: u64) {
    ...
    pub fn emit_turn_output(&mut self, turn_id: u64, is_final: bool) {
        ...
        if is_final {
            ...
            let Some(confirmed) = turn.into_draft().confirm_for_source(
        ...
        let Some(output) = draft.interim_output_for_source(
            self.source_identity(),
            self.counters.turn_session_id,
            turn_id,
            output_sequence,
    ...
    pub fn handle_open_turn_timeout(&mut self) -> bool {
    ...
    pub fn timeout_ticks(&self) -> u64 {
        timeout::ticks(
            self.config.segmentation.vad_interval_ms,
            self.config.turn.check_silence_ms,
        )
    }
}

impl RecognitionSession {
    pub fn process_pending_finalization_if_ready(&mut self) -> bool {
