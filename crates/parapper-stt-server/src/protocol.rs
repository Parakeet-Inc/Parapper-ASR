...
impl AudioFormat {
    #[cfg(test)]
    #[must_use]
    pub fn pcm_s16le_16khz_mono() -> Self {
        Self {
...
/// Parses and validates one client control message.
///
/// # Errors
///
/// Returns a fatal protocol error for invalid JSON, an unsupported version, or an invalid field.
pub fn parse_client_control(raw: &str) -> Result<ClientControl, ProtocolError> {
...
impl SessionProtocol {
    ...
    /// Applies one text control message to the session state machine.
    ///
    /// # Errors
    ///
    /// Returns a fatal protocol error when parsing fails or the action is invalid in this state.
    pub fn on_text(&mut self, raw: &str) -> Result<ProtocolAction, ProtocolError> {
    ...
    /// Validates one binary audio-frame length for the active session.
    ///
    /// # Errors
    ///
    /// Returns a fatal protocol error outside an active session or for an invalid frame size.
    pub fn on_binary(&mut self, byte_len: usize) -> Result<ProtocolAction, ProtocolError> {
    ...
    #[cfg(test)]
    pub fn on_disconnect(&mut self) -> ProtocolAction {
    ...
    #[cfg(test)]
    #[must_use]
    pub fn can_emit_turn_result(&self) -> bool {
        matches!(self.state, ProtocolState::Active | ProtocolState::Draining)
    }

    /// Marks a gracefully draining session as complete.
    ///
    /// # Errors
    ///
    /// Returns a fatal protocol error unless the session is currently draining.
    pub fn mark_done(&mut self) -> Result<(), ProtocolError> {
...
impl ServerMessage {
    ...
    pub fn error(session_id: Option<&str>, code: ErrorCode, message: impl Into<String>) -> Self {
        Self::Error {
            version: PROTOCOL_VERSION,
