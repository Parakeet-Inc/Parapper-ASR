...
use parapper_stt_engine::{
    AsrLanguage, RecognitionShutdownResult, RecognizedTextOutput, SAMPLE_RATE_HZ,
};
use tungstenite::{
    Message, WebSocket,
...
use super::{
    backend::{
        BackendStartError, InputSendError, RecognitionBackend, RecognitionBackendConfig,
        RecognitionStreamEvent, StartedRecognitionSession,
    },
    protocol::{
...
#[derive(Debug, Clone)]
pub struct StreamingRecognitionServerConfig {
    pub bind_addr: SocketAddr,
    pub api_key: Option<String>,
    /// Fixed host settings forwarded to every backend session.
    pub backend_config: RecognitionBackendConfig,
}

pub struct StreamingRecognitionServer {
...
impl StreamingRecognitionServer {
    /// Binds and starts the streaming recognition server with a host backend.
    ///
    /// # Errors
    ///
    /// Returns an error when the listener cannot bind/configure or its worker thread cannot start.
...
#[allow(
    ...
    reason = "the listener thread intentionally owns all accept-loop resources"
)]
fn run_accept_loop(
...
fn handle_connection(
    ...
    run_session(&mut websocket, backend, config.backend_config, shutdown);
...
#[allow(
    clippy::result_large_err,
    reason = "tungstenite requires ErrorResponse in the handshake callback"
)]
fn validate_upgrade(
    request: &Request,
    response: Response,
...
#[allow(
    ...
    reason = "the transport state loop mirrors the protocol actions in one auditable location"
)]
fn run_session(
    websocket: &mut WebSocket<TcpStream>,
    backend: &dyn RecognitionBackend,
    backend_config: RecognitionBackendConfig,
    shutdown: &AtomicBool,
) {
    ...
    loop {
        ...
        if shutdown.load(Ordering::Acquire) {
            if let Some(session) = active.as_mut() {
                session.recognition.input.take();
        ...
        match websocket.read() {
            Ok(Message::Text(text)) => {
                ...
                match action {
                    ProtocolAction::Start {
                        session_id, audio, ..
                    } => match backend.start(&session_id, &audio, backend_config) {
                        Ok(recognition) => {
                            ...
                            if send_message(websocket, &ServerMessage::ready(&session_id)).is_err()
                            {
                                cancel_active(&mut active);
                                return;
                            }
            ...
            Ok(Message::Binary(bytes)) => {
                ...
                let Some(input_sender) = session.recognition.input.as_ref() else {
                    return;
                };
                match input_sender.try_send(pcm_s16le_to_f32(&bytes)) {
                    Ok(()) => {}
                    Err(InputSendError::Overrun) => {
                        let _ = send_message(
                            websocket,
                            &ServerMessage::error(
                                Some(&session.session_id),
                    ...
                    Err(InputSendError::Disconnected) => {
                        let _ = send_message(
                            websocket,
                            &ServerMessage::error(
                                Some(&session.session_id),
    ...
    while let Ok(event) = session.recognition.event_receiver.try_recv() {
        match event {
            ...
            RecognitionStreamEvent::Output(output) => {
                send_message(
                    websocket,
                    &message_from_output(&session.session_id, *output),
                )?;
            }
        }
    }
...
pub(crate) struct FakeBackend {
    ...
    last_backend_config: std::sync::Mutex<RecognitionBackendConfig>,
    drain_input: bool,
    stop_delay: Duration,
...
impl FakeBackend {
    pub(crate) fn new(drain_input: bool) -> Arc<Self> {
        Arc::new(Self {
            ...
            last_backend_config: std::sync::Mutex::new(RecognitionBackendConfig::default()),
            drain_input,
    ...
    #[cfg(test)]
    fn last_backend_config(&self) -> RecognitionBackendConfig {
        *self.last_backend_config.lock().unwrap()
    }
}
...
impl RecognitionBackend for FakeBackend {
    fn start(
        &self,
        _session_id: &str,
        audio: &crate::protocol::AudioFormat,
        ...
        *self.last_backend_config.lock().unwrap() = config;
        self.starts.fetch_add(1, Ordering::AcqRel);
        let (input, input_receiver) = fake_audio_input(audio.sample_rate as usize * 2);
        ...
        let join = thread::spawn(move || {
            ...
            while !worker_stop.load(Ordering::Acquire) {
                ...
                match input_receiver.recv_timeout(Duration::from_millis(2)) {
                    Ok(_chunk) if !voiced => {
                        voiced = true;
                        let _ = event_sender.send(RecognitionStreamEvent::SpeechStarted);
                        let _ = event_sender.send(RecognitionStreamEvent::Output(Box::new(
...
struct FakeAudioInput {
    sender: mpsc::Sender<FakeAudioChunk>,
    queued_samples: Arc<AtomicUsize>,
    max_queued_samples: usize,
}

#[cfg(any(test, feature = "smoke-server"))]
struct FakeAudioChunk {
    _samples: Vec<f32>,
    queued_samples: Arc<AtomicUsize>,
    sample_count: usize,
}
...
#[cfg(any(test, feature = "smoke-server"))]
impl Drop for FakeAudioChunk {
    fn drop(&mut self) {
        self.queued_samples
            .fetch_sub(self.sample_count, Ordering::AcqRel);
    }
}

#[cfg(any(test, feature = "smoke-server"))]
fn fake_audio_input(max_queued_samples: usize) -> (FakeAudioInput, Receiver<FakeAudioChunk>) {
    let (sender, receiver) = mpsc::channel();
...
#[cfg(any(test, feature = "smoke-server"))]
impl super::backend::AudioInput for FakeAudioInput {
    fn try_send(&self, samples: Vec<f32>) -> Result<(), InputSendError> {
        ...
        if self
            ...
            .is_err()
        {
            return Err(InputSendError::Overrun);
        }
        let chunk = FakeAudioChunk {
            _samples: samples,
        ...
        self.sender
            .send(chunk)
            .map_err(|_| InputSendError::Disconnected)
    }
}
...
#[cfg(any(test, feature = "smoke-server"))]
impl super::backend::ActiveRecognitionSession for FakeActiveSession {
...
fn recognized_output(is_final: bool) -> RecognizedTextOutput {
    use parapper_stt_engine::{AsrModel, RecognitionSourceMeta, RecognizedTextMeta};
...
impl StreamingRecognitionServer {
    /// Starts a smoke-test server backed by [`FakeBackend`].
    ///
    /// # Errors
    ///
    /// Returns an error when the listener cannot bind or its worker thread cannot start.
    pub fn start_smoke(bind_addr: SocketAddr, api_key: Option<String>) -> Result<Self> {
...
mod tests {
    ...
    use super::*;
    use crate::protocol::MAX_AUDIO_FRAME_BYTES;
    ...
    #[test]
    fn server_fixed_tdt_dag_tuning_reaches_backend_but_is_not_client_controlled() {
        let backend = FakeBackend::new(true);
        let server = StreamingRecognitionServer::start_with_backend(
            StreamingRecognitionServerConfig {
                bind_addr: "127.0.0.1:0".parse().unwrap(),
                api_key: None,
                backend_config: RecognitionBackendConfig {
                    tdt_dag: Some(
                        crate::TdtDagDecodingConfig::new(
        ...
        socket
            .send(Message::Text(
                r#"{"version":1,"type":"session.start","session_id":"session-server-fixed","audio":{"encoding":"pcm_s16le","sample_rate":16000,"channels":1},"tdt_dag":{"beam_size":1,"ctc_gate_threshold":-1.0}}"#
                    .into(),
            ))
            .unwrap();
        assert_eq!(read_json(&mut socket)["type"], "session.ready");
        assert_eq!(
            backend.last_backend_config(),
        ...
        socket
            .send(Message::Text(
                r#"{"version":1,"type":"session.stop","session_id":"session-server-fixed"}"#.into(),
            ))
            .unwrap();
