...
<!-- cspell:words parapper Silero ReazonSpeech sherpa OSCQuery reqwest -->
...
```powershell
cargo run -p parapper --bin mock_ync_server -- --port 18080 --translate-delay-ms 0 --speech-delay-ms 4000
cargo run -p parapper --bin replay_mock_recognition -- --port 18080 --count 4 --interval-ms 0
```
...
- ASR 入力やターン単位の確定処理: [src-tauri/src/recognition/transcription/tests.rs](../src-tauri/src/recognition/transcription/tests.rs)
- VAD/Turn Detector の区切り、完了、再開時のイベント順序: [src-tauri/src/recognition/segment_builder/tests.rs](../src-tauri/src/recognition/segment_builder/tests.rs)
...
Turn Detector は `simple`, `namo` から選択できます。`simple` は調整可能な無音時間で完了を判定します。途中経過表示が有効な場合だけ、完了前の短い無音でそこまでの音声を ASR に渡します。`namo` は完了候補の ASR テキストを Namo に渡し、ASR 言語に応じて Japanese / English / Multilingual のモデルを使い分けます。
