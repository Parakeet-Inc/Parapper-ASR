...
const OUTPUT_FILE: &str = "reazon-fp32-full-prefix-beam8-raw-fixed-rescore.jsonl";
...
fn run(args: &Args) -> Result<()> {
    ...
    let summary = run_reazon_nbest_rescore(
        &args.manifest,
        &manifest,
        &mut engine,
        condition,
    ...
    writer.flush().context("failed to flush rescore output")?;
    println!(
        "{}",
...
struct CommandSummary<'a> {
    ...
    fixed_sequence_graph: &'static str,
    output: &'a Path,
    total: usize,
    completed: usize,
    failed: usize,
...
mod tests {
    ...
    #[test]
    fn cli_pins_width_eight_raw_rescoring_and_validates_sharding() {
        let args = Args::parse(
            [
                "--manifest",
                "data/manifest.json",
        ...
        assert_eq!((args.shard_index, args.shard_count), (3, 4));
    }
}
