...
// 4. interim_result_enabled かつ短い無音が interim_result_silence_ms まで続いたら
//    InterimResultSilenceReached で SegmentClosed を出し、そこまでの音声を ASR に渡して途中経過を表示する。
//    Turn は未完了なので、次 segment は同じ Turn の続きとして扱う。
        ...
        self.state = match state {
            SegmentBuilderState::Idle {
                ...
                if vad_result.is_speech {
                    ...
                    // pending_speech_chunks も捨て、次の発話に備えて pre_speech だけ更新する。
            ...
            SegmentBuilderState::Active(mut active) => {
                ...
                // - interim_result_silence_ms まで無音が続いたら、途中表示用にここまでを ASR へ渡す。
                // - 無音が続いた場合は発話完了として閉じる。
