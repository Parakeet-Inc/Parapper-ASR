...
#[cfg(feature = "real-asr-tests")]
use parapper_models::vad::VadEngine as _;
...
impl ReportingEngineAsrRunner {
    fn new(
        ...
        let (models, errors) = load_asr_models(&handle, config);
        let asr = AsrExecutionRuntime::new(models);
        assert!(
            errors.is_empty(),
...
impl AsrRequestRunner for ReportingEngineAsrRunner {
    fn set_normalize_input_audio(&mut self, enabled: bool) {
        self.config.asr.normalize_input_audio = enabled;
    }
...
fn run_fleurs_matrix_case(
    ...
    if spec.turn_detector == SystemTurnDetector::Namo {
        builder = builder.turn_decision_runner(Box::new(
            crate::recognition::EngineTurnDecisionRunner::new(&env.handle, &runner_config),
        ));
    }
...
fn replay_audio_through_real_vad(
    ...
    let mut vad = parapper_models::vad::OnnxRuntimeSileroVadEngine::new(
        vad_model,
