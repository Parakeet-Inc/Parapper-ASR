...
use parapper_stt_engine::{
    ...
    transcription::{
        ...
        task::{
            ...
            AsrTarget, AsrTaskKind, AudioRange, GlobalSampleIndex, TurnRevision, VadFrameIndex,
        },
    },
    turn::{GrammarBoundaryClass, Turn, TurnBoundaryCandidate, TurnDecision},
};
#[cfg(feature = "real-asr-tests")]
use parapper_stt_engine::AsrExecutionRuntime;
#[cfg(feature = "real-asr-tests")]
use crate::recognition::model_factory::load_asr_models;
...
impl RecognitionDriverHandle for RecordingRecognitionSession {
    fn update_runtime_parameters(
        &mut self,
        _parameters: parapper_stt_engine::SttRuntimeParameters,
    ) {
        self.calls.push(RuntimeCall::UpdateRuntimeParameters);
    }
...
struct RuntimeStateBuilder<'a> {
    runtime: &'a mut parapper_stt_engine::RecognitionSession,
}

fn runtime_state(
    runtime: &mut parapper_stt_engine::RecognitionSession,
) -> RuntimeStateBuilder<'_> {
...
fn replay_vad_frames_for_runtime_preserves_fifo_order_for_test_harness() {
    ...
    assert_eq!(
        runtime.calls,
        vec![
            RuntimeCall::UpdateRuntimeParameters,
