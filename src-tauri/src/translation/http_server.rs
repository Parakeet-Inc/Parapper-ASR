...
use super::provider::translate_local_text;
...
#[allow(
    ...
    reason = "field names are fixed by the OpenAI-compatible JSON contract"
)]
struct OpenAiUsage {
    prompt_tokens: u32,
...
fn accepts_local_translation_request(method: &Method, path: &str) -> bool {
    accepts_ync_request(method, path) || accepts_openai_request(method, path)
}

fn accepts_ync_request(method: &Method, path: &str) -> bool {
    method == &Method::Post && path == YNC_PATH
}

fn accepts_openai_request(method: &Method, path: &str) -> bool {
    method == &Method::Post && path == OPENAI_PATH
}
...
fn translation_response_for_path(
    ...
    match path {
        OPENAI_PATH => chat_completion_response(body, state, remote, path),
        YNC_PATH => neo_json_response(body, state, remote, path),
...
fn translation_prompt_instruction(text: &str) -> Option<&str> {
    ...
    for separator in ["\n\n", "```"] {
        if let Some((instruction, candidate)) = trimmed.rsplit_once(separator)
            ...
            && !strip_code_fence(candidate.trim()).is_empty()
        {
...
fn openai_response_log(response: &OpenAiChatCompletionResponse) -> String {
    let text_chars = response
        .choices
        .first()
        .map_or(0, |choice| choice.message.content.chars().count());
    format!(
...
mod tests {
    ...
    fn local_server_openai_chat_completion_empty_text_fails_without_translating() {
        ...
        assert!(error.clone().contains("empty"));
    ...
    fn ync_adapter_accepts_only_plugin_root_post() {
        assert!(accepts_ync_request(&Method::Post, "/"));
        assert!(!accepts_ync_request(&Method::Post, "/api/input"));
        assert!(!accepts_ync_request(&Method::Post, "/v1/chat/completions"));
        assert!(!accepts_ync_request(&Method::Get, "/"));
    }

    #[test]
    fn openai_adapter_accepts_only_chat_completions_post() {
        assert!(accepts_openai_request(
            &Method::Post,
            "/v1/chat/completions"
        ));
        assert!(!accepts_openai_request(&Method::Post, "/"));
        assert!(!accepts_openai_request(&Method::Post, "/api/input"));
        assert!(!accepts_openai_request(
            &Method::Get,
            "/v1/chat/completions"
        ));
